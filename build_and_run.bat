...
:: ANSI-Farben aktivieren (Windows 10 1511+)
...
echo %RED%[FEHLER]%RST% Unbekanntes Argument: %~1
goto :fatal
...
if defined VULKAN_SDK (
    ...
    if errorlevel 1 (
        echo %YLW%[ WARN ]%RST% VULKAN_SDK nicht gesetzt.
        echo        Falls Build fehlschlaegt: https://vulkan.lunarg.com/sdk/home
    ) else (
...
if not defined SKIA_DIR (
    ...
    echo %YLW%[ WARN ]%RST% SKIA_DIR nicht gesetzt — suche in %SKIA_DIR%
)

if not exist "%SKIA_DIR%\out\Release\skia.lib" (
    echo %RED%[FEHLER]%RST% Skia nicht gefunden: %SKIA_DIR%\out\Release\skia.lib
    echo.
    echo        Skia einmalig bauen:
    echo          git clone https://skia.googlesource.com/skia.git C:\skia
    echo          cd C:\skia ^& python tools\git-sync-deps
    ...
    echo.
    echo        Dann erneut starten:
    echo          build_and_run.bat --skia-dir C:\skia
    goto :fatal
)
...
if errorlevel 1 (
    ...
    echo          - Qt6_DIR falsch oder nicht gesetzt
...
:: ============================================================
echo.
echo %CYN%[4/5]%RST% Baue VulkanBrowser (%JOBS% Jobs)...
echo.

cmake --build "%BUILD_DIR%" --config %BUILD_TYPE% --parallel %JOBS%
if errorlevel 1 (
    echo.
    echo %RED%[FEHLER]%RST% Compile/Link fehlgeschlagen — Fehler siehe oben.
    goto :fatal
)

:: Binary lokalisieren (MSVC legt es in Release\ oder Debug\ ab, Ninja direkt)
set "BINARY="
...
if not defined BINARY (
    echo %RED%[FEHLER]%RST% vulkan-browser.exe nach dem Build nicht gefunden.
    goto :fatal
)
...
echo.
echo %GRN%[  OK  ]%RST% Binary: %BINARY%  (%BIN_MB% MB)
...
if "%NO_RUN%"=="1" (
    echo %YLW%[ SKIP ]%RST% --no-run gesetzt.
    goto :done
)

:: Vulkan Validation Layers im Debug-Build
if /i "%BUILD_TYPE%"=="Debug" (
    echo %YLW%[ INFO ]%RST% Vulkan Validation Layers aktiv
    set "VK_INSTANCE_LAYERS=VK_LAYER_KHRONOS_validation"
)

:: Qt-Plugins damit Qt seine DLLs findet
if defined Qt6_DIR (
...
echo.
echo %BLD%%GRN%============================================================%RST%
echo %BLD%%GRN%  Build erfolgreich — Browser laeuft jetzt.%RST%
echo %BLD%%GRN%============================================================%RST%
echo.
echo   Binary  : %BINARY%
...
echo   Skia    : %SKIA_DIR%
echo.
echo   Dieses Fenster bleibt offen bis der Browser geschlossen wird.
echo   Strg+C beendet Browser und Fenster sofort.
echo.

:: Browser OHNE start starten damit das CMD-Fenster offen bleibt
:: und stdout/stderr sichtbar sind (Vulkan-Fehler, Validation-Output)
"%BINARY%"
set "EXIT_CODE=%errorlevel%"

echo.
if "%EXIT_CODE%"=="0" (
    echo %GRN%[  OK  ]%RST% Browser normal beendet.
) else (
    echo %RED%[FEHLER]%RST% Browser beendet mit Exit-Code %EXIT_CODE%
    if "%EXIT_CODE%"=="1" echo        Moegliche Ursache: Kein Vulkan-faehiges GPU gefunden.
    if "%EXIT_CODE%"=="3" echo        Moegliche Ursache: Vulkan-Treiber abgestuerzt.
)
...
:: ============================================================
:: Fehler-Label: gibt Meldung aus, wartet auf Tastendruck
:: ============================================================
:fatal
echo.
echo %RED%══════════════════════════════════════════════════════%RST%
echo %RED%  BUILD FEHLGESCHLAGEN — Lies die Meldung oben.%RST%
echo %RED%══════════════════════════════════════════════════════%RST%
echo.
echo Druecke eine beliebige Taste zum Schliessen...
pause >nul
